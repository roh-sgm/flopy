...
Status: raw/text round-trip — **decision (Stage 3 Card 4): keep raw/text for
v1**. The raw-body round-trip already preserves multi-node `U1DINT`
continuation lines (tested); a semantic constructor is deferred. Status is
honest (no semantic authoring API).
...
[NIB1  node/head records]                 # cells set IBOUND=1
[NIBM1 node/head records]                 # cells set IBOUND=-1 (CHD-like)
[with transport: NICB0/NICB1/NICBM1 concentration blocks]
```

Rationale for raw/text v1: the `U1DINT` node lists are variable-length with
multiple nodes per continuation line — the exact construct the raw round-tripper
avoids mis-parsing — and TIB is normally generated by GUIs, not authored by
hand. Model A (≈112 k nodes, USG-T 1.8) round-trips TIB bit-for-bit through the
raw body. A semantic constructor would need to parse all of the above plus the
transport blocks; deferred until a target model needs from-scratch TIB.
