"""
mfusgdrn module. Contains the MfUsgDrn class.
"""

import numpy as np

from ..modflow.mfdrn import ModflowDrn
from ..pakbase import Package
from ..utils import MfList
from ..utils.recarray_utils import create_empty_recarray


class MfUsgDrn(ModflowDrn):
    """MODFLOW-USG Drain Package (DRN) with transport support.
    ...
    stress_period_data : dict, optional
        ...
        required base fields are ``node``, ``elev``, and ``cond``.
    ...
    extension : str
        Filename extension (default ``"drn"``).
    ...
    >>> import flopy
    >>> m = flopy.mfusg.MfUsg()
    >>> drn = flopy.mfusg.MfUsgDrn.load("MDV.drn", m, nper=12)
    """
    ...
    def __init__(
        ...
        # the CBC output file — mirrors ModflowDrn.__init__ exactly.
    ...
    def get_default_dtype(structured=True):
        if structured:
            return ModflowDrn.get_default_dtype(structured=True)
        return np.dtype([
            ("node", int),
            ("elev", np.float32),
            ("cond", np.float32),
    ...
    def get_empty(ncells=0, aux_names=None, structured=True):
        dtype = MfUsgDrn.get_default_dtype(structured=structured)
    ...
    def write_file(self):
        """Write the package file in MODFLOW-USG-T DRN format."""
        if self.parent.structured:
            ModflowDrn.write_file(self)
            return
    ...
    def load(cls, f, model, nper=None, ext_unit_dict=None, check=True):
        """Load a MODFLOW-USG-T DRN package from file.

        Parameters
        ----------
        f : str or file-like
            Path to the .drn file, or an open file handle.
        model : MfUsg
        ...
        """
        if model.structured:
            return ModflowDrn.load(f, model, nper=nper, ext_unit_dict=ext_unit_dict, check=check)

        if model.verbose:
            print("loading mfusg drn package file...")
        if nper is None:
            nper = model.nper
        ...
        # Parse header: MXACTD IDRNCB [AUX varname ...]
        tokens = line.split()
