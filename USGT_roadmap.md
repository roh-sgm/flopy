...
| SGB | `SGB` | `MfUsgSgb` | ✅ Full (authoring) / Parameter-preserving (NPSGB) / Expanded valid write (list controls) | `glo2sgbu1.f` | Specified Gradient Boundary. Node-based `(node, gradient)` list, AUX transport concentrations, `ITMP/-1` reuse; 0-based internal / 1-based file. Reads `SFAC` (inert on gradient per Fortran ISCLOC), `OPEN-CLOSE`, `EXTERNAL` list controls (expanded inline on write). **Stage 4.4C: `NPSGB>0` list parameters are now preserved** (load → write → reload keeps the `PARAMETER NPSGB MXS` record, definitions with `NLST` `NODE GRADIENT [aux]` rows, and per-SP `ITMP NP` + active names; uses the shared list-parameter helpers in `_usgt_parameters.py`, `UPARLSTAL`/`UPARLSTRP`/`UPARLSTSUB`). Not `Full` on the parametric axis: from-scratch parameter authoring and parameter `INSTANCES` (Fortran-supported) raise `NotImplementedError`; inconsistent state raises `ValueError`. Registered in `MfUsg.load()`. See `USGT_STAGE4_04_PARAMETERS_SGB.md`. **Verified** |
