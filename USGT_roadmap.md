...
| EVT | ✅ (intentionally not Full) | EVT Fullness Card B (Stage 4): all NEVTOP modes (struct + unstruct), transport IETFACTOR 0/<0/>0, per-SP reuse authored + round-trip tested; IETFACTOR round-trip bug fixed; exe smoke added. **Stage 4.6F-A: NPEVT EVTR array parameters preserved + authored from scratch** (`parameters=`/`evtr_parm=`, PARAMETER line, INSTANCES; `expand_parameters=True` keeps legacy NPEVT=0). Remaining gap keeping it not-Full: ETS zonal time-series raises `NotImplementedError` (Stage 4.6F-B). See `USGT_STAGE4_EVT_FULLNESS.md`. |
