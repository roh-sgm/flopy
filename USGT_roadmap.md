...
| OC | `OC` | `MfUsgOc` | ✅ | — | ATS adaptive time-stepping (`ATSA`) authoring + round-trip tested; `BOOTSTRAPPING` parsed on load. Full output-block round-trip not exhaustively tested → kept `✅`, not `Full` (Stage 3 Card 8). |
...
| EVT | `EVT` | `MfUsgEvt` | ✅ | `gwf2evt8u1.f` | Transport ET factor (`IETFACTOR`/`ETFACTOR`) authoring tested; load/write exercised across the suite. Full per-`NEVTOP` array authoring not exhaustively tested → kept `✅`, not `Full` (Stage 3 Card 8). **Verified** |
...
| MDT | `MDT` | `MfUsgMdt` | ✅ | `gwt2mdtu1.for` | Matrix Diffusion Transport. Load/write **round-trip validated via the three Ex7 Matrix-Diffusion real models** (SandTank, Multispecies, DiscreteFracture). Field order not yet independently Fortran-audited and no from-scratch authoring test → kept `✅`, not `Full` (Stage 3 Card 8). |
...
| GSF | `GSF` | `MfUsgGsf` | ✅ Full (authoring) | — | Grid Specification File (not solver input — consumed by post-processors / `UnstructuredGrid.from_gridspec`, so no Fortran reader). Stage 4.2: semantic `vertices` + `node_data` authoring and `parse=True` load — header (`UNSTRUCTURED [GWF]`), `nnodes`/`nlay`, vertex `(x,y,z)`, per-node `(node, xc, yc, zc, layer, vertices)`; 0-based internal nodes/vertices/layers, 1-based file ids; trailing line-2 gridgen flags preserved (default `(1,1)`). `from_grid(model, grid, zverts)` builds from an `UnstructuredGrid` (requires per-vertex z, which the grid does not retain — fails explicitly otherwise). `to_grid()` still delegates to `from_gridspec`. `load` defaults to byte-faithful raw round-trip with raw fallback on unparseable files. From-scratch authoring, write/reload, 0-based↔1-based, `to_grid`, invalid-ref, and mode-exclusivity tests. **Verified (Stage 4.2)** |
| SFR | `SFR` | base `ModflowSfr2` | Compatibility-only | `gwf2sfr7u1.f` | Base MODFLOW-2005 class; USG-T unstructured node-indexed items not validated. Not used by any target/example USG-T model (CLN is the preferred coupling). Out of scope (Card 6). |
| STR | `STR` | base `ModflowStr` | Compatibility-only | `gwf2str7u1.f` | Base class; USG-T format not validated. Not used by any target/example model. Out of scope (Card 6). |
...
| SUB | `SUB` | base `ModflowSub` | Compatibility-only | `gwf2sub7u1.f` | Base class; USG-T items not validated. Not used by any target/example model. Out of scope (Card 6). |
| SWT | `SWT` | base `ModflowSwt` | Compatibility-only | — | Base class; USG-T format not validated. Not used by any target/example model. Out of scope (Card 6). |
