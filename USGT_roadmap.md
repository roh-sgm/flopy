...
| QRT | `QRT` | `MfUsgQrt` | ✅ Full (authoring) / Parameter structural-preserving (NPQRT, not execution-guaranteed) / Expanded valid write (list controls) | `gwf2QRT8u.f` | Sink with Return Flow. Node-based `(node, q, rfprop)` + variable-length recipient-node lists (`NodQRT` via U1DINT), `CHANGEC`/`IQCHNGTYP` transport, AUX, `ITMP/-1` reuse. Reads `SFAC` (scales Q)/`OPEN-CLOSE`/`EXTERNAL` (expanded inline on write); `recipient_nodes` validated against the stress list. `AUTOFLOWREDUCE` preserved. **Stage 4.4E: `NPQRT>0` list parameters are structurally preserved** (load → write → reload of definitions + recipient blocks + per-SP `ITMP NP` activations; `MXAQRT` covers the active total; `MXRTCELLS` reflects definition recipients). QRT is type-consistent (`PARTYP='QRT'` both sides, `gwf2QRT8u.f:183`/`:1118`), but **not execution-guaranteed**: the parameter value scales `QRTF(5)=NumRT` not `Q` (`IPVL=5`; a Fortran bug — `SFAC` scales `Q` at `ISCLOC=4`), and `NodQRT` is not copied on activation. From-scratch authoring and `INSTANCES` raise `NotImplementedError`; `TRANSIENTQ` still fails explicitly; `MXL`/consistency validated. Registered in `MfUsg.load()`. See `USGT_STAGE4_04_PARAMETERS_QRT.md`. **Verified** |
